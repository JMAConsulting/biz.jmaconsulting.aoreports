...
WHERE  civicrm_contribution_contribution_id={$row['civicrm_contribution_contribution_id']}";
    ...
    if (!empty($this->_params['fields']['issued_on']) ||
      ...
      $this->_from .= "
        LEFT JOIN cdntaxreceipts_log_contributions cdn_con
          ON cdn_con.contribution_id = {$this->_aliases['civicrm_contribution']}.id
        LEFT JOIN cdntaxreceipts_log {$this->_aliases['cdntaxreceipts_log']}
          ON {$this->_aliases['cdntaxreceipts_log']}.id = cdn_con.receipt_id";
    }
    // JMA - Add join to groups table
    if (!empty($this->_params['fields']['title'])) {
      $this->_from .= "
        LEFT JOIN civicrm_group_contact cg
          ON cg.contact_id = {$this->_aliases['civicrm_contact']}.id AND cg.status = 'Added'
        LEFT JOIN civicrm_group {$this->_aliases['civicrm_group']}
          ON {$this->_aliases['civicrm_group']}.id = cg.group_id";
    }
  ...
  public function buildGroupTempTable($override = FALSE) {
    ...
    $query = "
       ...
       INNER JOIN civicrm_contribution cc ON cc.contact_id = group_contact.contact_id
       WHERE $whereUsed
