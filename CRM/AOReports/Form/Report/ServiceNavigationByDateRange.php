...
class CRM_AOReports_Form_Report_ServiceNavigationByDateRange extends CRM_AOReports_Form_Report_FamiliesServed {
  ...
  function from() {
    if (!empty($this->_params['dof_relative']) || !empty($this->_params['dof_from']) || !empty($this->_params['dof_to'])) {
    ...
    foreach ($newRows as $key => $row1) {
      foreach ($rows as &$row) {
        ...
        elseif ($row['civicrm_contact_family_count'] == '') {
          $newRows['no-region']['civicrm_contact_total_count'] = $row['civicrm_contact_total'];
        }
      }
    }
